...
public class CandidateDaoImpl implements CandidateDao{
	...
	public List<Candidate> getCandidateByPhoneNumber(String phoneNumber) {
		...
		try {
			Session session=sessionFactory.getCurrentSession();
			return session.createQuery(GET_CANDIDATE_BYPHONE,Candidate.class).setParameter(1,phoneNumber).getResultList();
		} catch (Exception e) {
			logger.error(e.getMessage());
	...
	public List<Candidate> getCandidateByName(String name) {
		...
		try {
			Session session=sessionFactory.getCurrentSession();
			return session.createQuery(GET_CANDIDATE_BYNAME,Candidate.class).setParameter(1, "%"+name+"%").getResultList();
		} catch (Exception e) {
			logger.error(e.getMessage());
			throw new DatabaseException(ERROR_IN_READING);
		}
	...
	public List<Candidate> getCandidateByExperience(String exp) {
		...
		try {
			Session session=sessionFactory.getCurrentSession();
			return session.createQuery(GET_CANDIDATE_BYEXPERIENCE,Candidate.class).setParameter(1,exp).getResultList();
		} catch (Exception e) {
			logger.error(e.getMessage());
			throw new DatabaseException(ERROR_IN_READING);
		}
	...
	/*
	 *The logic is to set dueDate as 'today minus 3 months', first it checks for applied job role, if matches,
	 * then it checks for applied date, we cannot apply if applied date is greater than or equal to dueDate(which is today minus 3 months).
	 * If result set is null then we can apply for the role that means a candidate didn't apply for 
	 * the role or applied but it satisfies dueDate condition
	 */
	@Override
	public Candidate validateJobRole(CandidateDto candidateDto) {
		...
		try {
			...
			return session.createQuery(VALIDATE_JOBROLE,Candidate.class)
					...
					.setParameter("role",candidateDto.getJobRole())
					.setParameter("dueDate",today.minusMonths(3))
					.getSingleResult();
